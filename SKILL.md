...
1. 运行命令获取数据：
```bash
python ${SKILL_DIR}/scripts/ssh_config_manager_v3.py list-servers
```
2. 解析返回的 JSON 数据
...
**MUST**: 使用 `python ${SKILL_DIR}/scripts/脚本名.py` 格式。使用别名（alias）标识服务器。

**NEVER**: 调用python脚本的路径必须使用 `/` 作为分隔符，禁止使用 `\`；不要使用 `cd` 到脚本目录再执行; 不要直接写 `ssh` 或 `scp` 命令。

### 执行远程命令

```bash
# 方式1：直接传入命令（简单命令推荐）
python ${SKILL_DIR}/scripts/ssh_execute.py <别名> "<命令>"

# 方式2：从文件读取命令（复杂命令推荐，避免参数解析错误）
python ${SKILL_DIR}/scripts/ssh_execute.py <别名> --command-file <命令文件路径>
```

可选参数：`--timeout <秒>` `--no-daemon`

**推荐**：对于包含特殊字符（如 `|`, `>`, `<`, `&`, `;`, 引号等）的复杂命令，建议先将命令写入临时文件，然后使用 `--command-file` 参数传入文件路径，避免参数解析错误。
...
```bash
MSYS_NO_PATHCONV=1 python ${SKILL_DIR}/scripts/ssh_upload.py <别名> "<本地路径>" "<远程路径>"
```
...
```bash
MSYS_NO_PATHCONV=1 python ${SKILL_DIR}/scripts/ssh_download.py <别名> "<远程路径>" "<本地路径>"
```
...
MSYS_NO_PATHCONV=1 python "${SKILL_DIR}/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>"

# 强制直连模式（大文件推荐，数据直接在服务器间传输）
MSYS_NO_PATHCONV=1 python "${SKILL_DIR}/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --mode direct

# 强制流式转发（小文件或服务器间网络不通时）
MSYS_NO_PATHCONV=1 python "${SKILL_DIR}/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --mode stream

# 混合模式（先尝试直连，失败后自动降级到流式）
MSYS_NO_PATHCONV=1 python "${SKILL_DIR}/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --mode hybrid

# 使用 rsync（仅直连模式，支持增量同步）
MSYS_NO_PATHCONV=1 python "${SKILL_DIR}/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --use-rsync
```
...
```bash
# 对所有服务器执行（直接命令）
python "${SKILL_DIR}/scripts/ssh_cluster.py" "<命令>" --parallel

# 对指定别名列表执行（直接命令）
python "${SKILL_DIR}/scripts/ssh_cluster.py" "<命令>" --hosts "DEV-002,DEV-003" --parallel

# 按环境过滤（直接命令）
python "${SKILL_DIR}/scripts/ssh_cluster.py" "<命令>" --environment production --parallel

# 对所有服务器执行（从文件读取命令，复杂命令推荐）
python "${SKILL_DIR}/scripts/ssh_cluster.py" --command-file <命令文件路径> --parallel

# 对指定别名列表执行（从文件读取命令）
python "${SKILL_DIR}/scripts/ssh_cluster.py" --command-file <命令文件路径> --hosts "DEV-002,DEV-003" --parallel

# 按环境过滤（从文件读取命令）
python "${SKILL_DIR}/scripts/ssh_cluster.py" --command-file <命令文件路径> --environment production --parallel

# 按标签过滤
python "${SKILL_DIR}/scripts/ssh_cluster.py" "<命令>" --tags "web,nginx" --parallel
```
...
# 按环境过滤
python "${SKILL_DIR}/scripts/ssh_config_manager_v3.py" list-servers --environment production

# 查找服务器（支持别名和描述模糊查找）
python "${SKILL_DIR}/scripts/ssh_config_manager_v3.py" find "<关键词>"

# 创建配置
python "${SKILL_DIR}/scripts/ssh_config_manager_v3.py" create --alias <别名> --host <IP> --user <用户名> --key <密钥文件> --environment <环境>

# 更新配置（只更新提供的字段，其他字段保持不变）
python "${SKILL_DIR}/scripts/ssh_config_manager_v3.py" update <别名> --description "新描述" --tags tag1 tag2 tag3
python "${SKILL_DIR}/scripts/ssh_config_manager_v3.py" update <别名> --environment production --location "新位置"
python "${SKILL_DIR}/scripts/ssh_config_manager_v3.py" update <别名> --host <新IP> --port <新端口>

# 删除配置
python "${SKILL_DIR}/scripts/ssh_config_manager_v3.py" delete <别名>
```
...
```bash
# 启动 tunnel（自动分配本地端口）
python "${SKILL_DIR}/scripts/ssh_tunnel.py" start <别名> --remote-port <端口>

# 指定本地端口
python "${SKILL_DIR}/scripts/ssh_tunnel.py" start <别名> --local-port <本地端口> --remote-port <远程端口>

# 转发到远程的其他主机
python "${SKILL_DIR}/scripts/ssh_tunnel.py" start <别名> --remote-host <远程主机> --remote-port <端口>

# 列出所有活动的 tunnel
python "${SKILL_DIR}/scripts/ssh_tunnel.py" list

# 查看 tunnel 状态
python "${SKILL_DIR}/scripts/ssh_tunnel.py" status <tunnel-id>

# 停止 tunnel
python "${SKILL_DIR}/scripts/ssh_tunnel.py" stop <tunnel-id>

# 停止服务器的所有 tunnel
python "${SKILL_DIR}/scripts/ssh_tunnel.py" stop-all <别名>
```
...
```bash
# 启动守护进程（通常不需要手动启动）
python "${SKILL_DIR}/scripts/ssh_daemon.py" start <别名>

# 查看守护进程状态
python "${SKILL_DIR}/scripts/ssh_daemon.py" status <别名>

# 停止守护进程
python "${SKILL_DIR}/scripts/ssh_daemon.py" stop <别名>
```
...
```bash
# 好：一次调用获取多个信息
python ${SKILL_DIR}/scripts/ssh_execute.py DEV-002 "hostname && uptime && df -h && free -m"

# 差：多次调用分别获取
python ${SKILL_DIR}/scripts/ssh_execute.py DEV-002 "hostname"
python ${SKILL_DIR}/scripts/ssh_execute.py DEV-002 "uptime"
python ${SKILL_DIR}/scripts/ssh_execute.py DEV-002 "df -h"
python ${SKILL_DIR}/scripts/ssh_execute.py DEV-002 "free -m"
```
...
```bash
python "${SKILL_DIR}/scripts/ssh_execute.py" <别名> "<命令>" --no-daemon
```
...
**交互式 Shell**：本 skill 不支持交互式 SSH 会话（如 `ssh user@host` 后手动输入命令）。所有命令必须通过脚本一次性执行并获取结果。这是 opencode 环境的限制，无法实现实时交互式终端。
